...
state, country, and over time. This is our contribution.\\

Analysis of multiple predictors of happiness in addition to energy use would
substantially change the focus. We believe we use best and most comprehensive
data available for energy use and happiness--addition of controls will severely
limit our coverage as explained below.

More importantly, as reviewer also points out, it
wouldn't help much, as there is endogeneity, and as explained below, our search for
appropriate natural experiment will take many more months, and we believe our
findings are timely and informative enough to warrant publication.
%
Indeed, there is barely any other topic that is more important than climate
change or energy use and yet there is also hardly any other topic that received
less  attention with respect to happiness. 
...
% Thank you for suggestion to include corruption, social capital, religion,
% inequality. We do think that is a great idea. Again, we decided to postpone it for
% the follow-up paper for several reasons.\\
Again, we are interested in bivariate relationship, but we did try it--we added religiosity and
social capital from World Values Survey (WVS), and Gini and corruption from the World
Bank, however, social capital cuts sample size by about half. Likewise, 
availability of Gini for years of survey in WVS is limited. Again, the goal of
this study is breadth, not depth, we want to cover as many areas as possible.

\cc{Energy use is endogenous, is likely to be affected by income per capita, how can the authors deal with this issue?}

We agree that endogeneity is important. We cannot tackle it using data
available. While we do not tackle it in this
study, we do not sweep under the rug, and we acknowledge it now. 
We agree that energy use is endogenous. How would we try to deal with it?
 Following \citep{sorensen12}, we would
 try to find a way to argue that variation in energy use was independent from
 controls. We are not sure what variables could be good instruments, but we
 think we may be able to find some natural experiment, for instance, when energy
 consumption was disrupted by natural disaster, or power plant or electric grid
 failure. We are also thinking to look at oil consumption and natural gas
 consumptions--embargoes, shortages, dramatic price increases, etc. It is,
 however, multi-month, possibly multi-year, research. 
...
doing a quick and possibly sloppy analysis, we think it is more useful to present simple current
...
present simple linear fit (bivariate), or quadratic fit, both without any other
controls included. Only in the appendix there were some initial results using
multivariate regressions. We clarify it now in text and to avoid confusion we
drop regressions from the supplementary material--again, we believe it is better
...
paper. And we also include a brief discussion making the case that our
...
TODO Micah %or Rubia or David Salas (he studies energy!)
...
Yes, indeed, this is a truly great point--we overlooked importance of that
point, we emphasize it much
better now. We even changed the title and abstract to point to centrality of
this finding. We thank you very much for this point! We add more discussion in
text--see tracked changes.
Yet we do not make dramatic updates as we want to keep direct focus
on energy use, not economic growth. We add in appendix a graph for gdp against energy.

Though our main argument is cross-sectional, not time-series, and we now also
added time series graphs  by census regions in online supplementary material.  
...
Graphs are omitted from main text for few reasons--again, paper is mainly about
energy use and not GDP; also, one reason is that there is overlap between observations and graph needs to be really big. Per time-series graphs--analytical part of paper is mainly about cross-sectional results.
...
on regression fit and making the relationship stronger. We did not mean that these
places show positive relationship, just simply meant that they are high on both,
...
Another great point! We dropped it altogether and we will make another paper out of it.
...
footnote with clarification--nevertheless, we decided to leave Sen because that
paragraph discusses HDI, and hence Sen is relevant to the discussion.
