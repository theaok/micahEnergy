...
% TODO 
%
% - Make sure paper actually does these things...
...
fossil fuels, which will run out. %TODO be specific give numbers
...
Why energy use is not associated with mnore happiness in the US? There are
several realted explanations. 
A second possible cause is a ``hedonic treadmill'' \cite{brickman78cj} -- increased consumption does not necessarily
increase well-being due to hedonic-adaptation: In other words, more stuff
...
in the US--poor countries need to consume more enrgy, but not the rich
countries, in order to increase their happiness.
...
things. For instcnace, bowling, yoga, gardening result in more happiness than
extra bedroom in a house. TODO: cite 
Another illustrative finding is that of consumption. For most people,
...
distance. Will it result in happiness? No.
 We get
used to big houses, but not to commute \cite{stutzer03,kahneman04}. 
 Any luxury consumption (not only big houses) % SUVs, LV handbags
 is not likely to result in
lasting happiness because luxuries  are positional goods--you buy them to have a better position in a
society, or to show that you are better than others--not to have a better quality of life.  The problem with
positional goods is that acquiring them leads to consumption
arms-race. You cannot win--you are  on hedonic treadmill. % --we compare to others and, we
...
% blocks of American suburbia.
 Even worse, we adapt to luxuries such as  big houses--they don't make us any happier  after some
time, and in a vicious cycle we desire ever bigger houses. But
we don't adapt to our commutes--it consistently makes us
 unhappy \cite{frank05}.

%aok: micah, not sure how much elaboration %we need here

A useful concept of expected v experience utility
helps to illustrate it
\cite{kahneman97ws} in figure \ref{fUT}. We
decide on much of our consumption (energy, cars, houses, etc) and income (how hard and how long to work) based on
expected utility (or happiness)--and we are often (predictably) wrong--we think that higher income and consumption  will
make  us happier than they actually do. A related  explanation can be
 advanced  in terms of income or consumption aspirations in figure \ref{fAS}.  

\begin{figure}[H]
  \begin{centering}
    \includegraphics[height=2.7in]{graphsAndTables/utility1}
...
\begin{figure}[H]
  \begin{centering}
    \includegraphics[height=2.7in]{graphsAndTables/asp_inc1}
    \caption{Income (or consumption) and life satisfaction (or
      happiness) given aspirations. Income increases happiness given some aspirations,
      but as we increase our income, aspirations curve shifts from
      $A_1$ to $A_2$, and hence, the
      resulting happiness is lower than expected.}\label{fAS}
  \end{centering}
\end{figure}
...
happen. %http://www.eia.gov/state/rankings/
