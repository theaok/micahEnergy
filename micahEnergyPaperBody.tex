...
Consumption beyond a point does not buy happiness. It just buys position, but
because everybody tries to oucompete everyone else, this race cannot be won.
A related cause is a ``hedonic treadmill'' \cite{brickman78cj} -- increased consumption does not necessarily
...
decide on much of our consumption (energy, houses, etc) and income (how hard and how long to work) based on
expected utility (or happiness). We are often (predictably) wrong--we think that higher income and consumption  will
...
%TODO: drop one or combine into one fig with 2 panels
\begin{figure}[H]
  \begin{centering}
